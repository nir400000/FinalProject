...
```bash
sudo cp polkit/50-babymonitor-wifi.rules /etc/polkit-1/rules.d/
...
Edit `sudoers/babymonitor-nmcli` (replace `USER` with your username), then:

```bash
sudo cp sudoers/babymonitor-nmcli /etc/sudoers.d/babymonitor-nmcli
